...
for cls in args.cls:
    norm_areas[cls] = {
        i: [((x[3] - x[1]) / w, (x[4] - x[2]) / h) for x in boxes[cls][i]]
        for i in range(num_images)
    }
...
for cls in args.cls:
    selected_areas[cls] = {
        i: [x for x in norm_areas[cls][i] if x[0] >= args.w_thr and x[1] >= args.h_thr]
...
for cls in args.cls:
    corr_imgs[cls] = {
        i: selected_areas[cls][i]
        for i in range(num_images)
        if selected_areas[cls][i] != []
    }
